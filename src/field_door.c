...
// emerald+: both regions' door animations are compiled (Kanto postgame).
#if 1
...
// emerald+: both regions' door palettes are compiled (Kanto postgame).
#if 1
...
// emerald+: both regions' doors are listed. Lookup is keyed on (metatileNum, tileset),
// so the two halves cannot alias each other even where metatile numbers coincide —
// GetDoorGraphics only matches an entry whose tileset is one the current map loaded.
